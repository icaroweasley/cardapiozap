...
export default function BroadcastManager({ setActiveTab }: { setActiveTab?: (tab: 'SETTINGS' | 'KANBAN' | 'PRODUCTS' | 'BROADCAST' | 'ADMIN') => void }) {
  ...
  const [activeTargetTab, setActiveTargetTab] = useState<'ORIGIN' | 'TARGET'>('ORIGIN');
        ...
        {actualInstanceStatus === 'Conectado' && currentScreen === 2 && (
          <div className="flex-1 flex flex-col w-full h-full lg:overflow-hidden animate-fade-in font-inter">
            ...
            <div className="flex w-full mb-4 bg-white/40 dark:bg-black/30 backdrop-blur-xl border border-black/10 dark:border-white/10 rounded-2xl overflow-hidden p-1 shrink-0 shadow-sm lg:hidden">
              ...
              <button 
                onClick={() => setActiveTargetTab('TARGET')}
                className={`flex-1 py-3 md:py-4 text-[10px] md:text-xs font-bold uppercase tracking-widest rounded-xl transition-all ${activeTargetTab === 'TARGET' ? 'bg-black dark:bg-white text-white dark:text-black shadow-md' : 'text-black/50 dark:text-white/50 hover:bg-black/5 dark:hover:bg-white/5'}`}
              >
                2. Lista Alvo ({targetContacts.length})
              </button>
            </div>
                    ...
                    <div className="flex-1 overflow-y-auto p-3 hide-scrollbar flex flex-col gap-2 min-h-0 relative">
                      {allContacts.length === 0 ? (
                        <div className="h-full flex flex-col items-center justify-center text-black/40 dark:text-white/40 text-center p-6">
                          ...
                          <p className="text-sm font-inter font-bold">Nenhum contato carregado.</p>
                          <p className="text-xs mt-2 opacity-70">Clique no botão "Importar do WhatsApp" acima.</p>
                        </div>
                      ) : (
                        <>
                  ...
                  <div className="relative z-10 flex flex-col h-full w-full">
                    ...
                    <div className="flex-1 overflow-y-auto p-3 hide-scrollbar flex flex-col gap-2 min-h-0 relative">
                      {targetContacts.length === 0 ? (
                        <div className="h-full flex flex-col items-center justify-center text-black/40 dark:text-white/40 text-center p-6">
                          <Users size={48} className="mb-4 opacity-30" />
                          <p className="text-sm font-inter font-bold">Sua lista alvo está vazia.</p>
                          <p className="text-xs mt-2 opacity-70">Volte para a aba "Selecionar Contatos" para adicionar.</p>
                        </div>
                      ) : (
                        <>
