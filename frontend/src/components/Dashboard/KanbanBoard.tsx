...
import { RefreshCw, MessageCircle, DollarSign, ShoppingBag, TrendingUp, Clock, Printer, XCircle, ChevronsLeft, ChevronsRight } from 'lucide-react';
...
export default function KanbanBoard() {
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-white/40 dark:bg-black/40 backdrop-blur-3xl border border-black/10 dark:border-white/10 shadow-2xl overflow-hidden animate-fade-up rounded-3xl lg:m-2">
      ...
      <div className="md:hidden flex items-center justify-center gap-2 mt-4 text-[10px] uppercase font-bold tracking-widest text-black/40 dark:text-white/40">
        <ChevronsLeft size={14} className="animate-pulse" />
        Deslize para ver colunas ocultas
      ...
      <div className="flex-1 overflow-x-auto p-4 md:p-6 hide-scrollbar snap-x snap-mandatory">
        <div className="flex gap-4 md:gap-6 h-full items-stretch">
          {COLUMNS.map(column => (
            <div key={column.id} className="flex-none w-full md:flex-1 md:w-auto md:min-w-0 flex flex-col bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 h-full backdrop-blur-md rounded-2xl overflow-hidden snap-center">
