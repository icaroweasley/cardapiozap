        ...
        {products.map((product) => {
          ...
          >
            ...
            <div className="flex items-center justify-between border-t border-black/10 dark:border-white/10 pt-5 mt-auto h-12" onClick={e => e.stopPropagation()}>
              ...
              {isJustAdded ? (
                <button className="font-inter text-[10px] tracking-[0.2em] font-bold uppercase text-white border border-green-500/50 px-4 py-2 bg-green-500 transition-all shadow-[0_0_15px_rgba(34,197,94,0.4)] flex items-center justify-center rounded-xl">
                  ✓ ADICIONADO
                </button>
              ) : items.find(i => i.productId === product.id) ? (
                <div className="flex items-center gap-3 border border-green-500/50 bg-green-500 text-white backdrop-blur-md p-1 shadow-[0_0_15px_rgba(34,197,94,0.3)] rounded-xl">
                  <button 
                    onClick={() => { 
                      const item = items.find(i => i.productId === product.id);
                      if (item) updateQuantity(product.id, item.quantity - 1); 
                    }} 
                    className="p-1.5 text-white/80 hover:text-white hover:bg-black/20 transition-colors rounded-lg"
                  >
                    <Minus size={14} />
                  </button>
